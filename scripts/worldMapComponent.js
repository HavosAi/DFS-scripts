  ...
  sheet.forEach((row, rowIndex) => {
    if (rowIndex >= 1) {
      ...
      row.forEach((cell, cellIndex) => {
        ...
        if (
          sheet[0][cellIndex]?.trim() === config.sheetColumnTitleOfStudyCountry
        ) {
        ...
        study.filtersBlockArray.forEach((filter) => {
          if (cell && sheet[0][cellIndex]?.trim() === filter.sheetColumnTitle) {
            cell?.split(valueSeparator).forEach((cellValue) => {
      ...
      if (study.studyTitle.length > 0 && study.studyCountriesArray.length > 0) {
...
async function getGoogleSheet(
  ...
  try {
    const sheet = await fetch(
      `https://agricultureinthedigitalage.org/wp-json/google-sheets-api/get-data?tab=${
