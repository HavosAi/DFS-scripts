...
var waitForResources = setInterval(function () {
  if (typeof jQuery != 'undefined' && typeof document.multiselect === "function") {
      ...
      function createMapVizualization(worldMapElement) {
        ...
        const filterSelectClass =
          mainComponentId + "-visualization-filter-select";
      ...
      function getConfig(settings) {
        //||column title for 'Study title'||column title for 'Country'||column title for 'Study link'||Filter titles||column titles for 'Filters'    ||Filter default values||Table columns||column title for 'Table columns'||
        ...
        const sheetColumnNumberOfTableColumnTitleLetter = 7;
        ...
        settings.forEach((row, rowIndex) => {
          ...
          if (rowIndex >= 1) {
            ...
            if (
              tempTableColumnItem.title &&
              tempTableColumnItem.sheetColumnTitle
            ) {
      ...
      function getValuesForFiltersArray(studiesArray, filtersBlockArray) {
        ...
        studiesArray.forEach((study, index, array) => {
          study.filtersBlockArray.forEach(
            (studyFilterItem, indexOfblockFilter) => {
              studyFilterItem.values.forEach((studyFilterItemValue) => {
                filtersValuesFromSheetArray.forEach((filterArrayTemp) => {
                  if (filterArrayTemp.title == studyFilterItem.title) {
                    if (
                      filterArrayTemp.values.indexOf(studyFilterItemValue) ===
                      -1
                    ) {
      ...
      function createTableComponent(
        ...
        tableWrapperBlockButtonElement.append(
          tableWrapperBlockButtonLinkElement
        );
      ...
      function createFiltersBlockComponent(
        ...
        filtersBlockArray.forEach(
          (filtersBlockArrayItem, indexOfblockFilter) => {
            ...
            filtersValuesFromSheetArray.forEach(
              (filtersValuesFromSheetArrayItem) => {
                if (
                  ...
                  filtersValuesFromSheetArrayItem.values.forEach(
                    (filterValue) => {
                      let isValueAlreadyAdded = false;
                      Object.values(filterSelectElement.options).forEach(
                        (existedOptionValue) => {
                          if (
                            existedOptionValue.innerHTML == filterValue.trim()
                          ) {
      ...
      function highlightFilter() {
        ...
        if (highlightId) {
          document
            ...
            .classList.add("highlighted-filter");
        }
      }
