...
evidenceGapMapElementsArray.forEach((evidenceGapMapElement) => {
  ...
  // WARNING!!! VALUES HARDCODED!!! NEED TO UPDATE LOGIC FOR GOOGLE SHEETS API v.4 !!!

  getHardcodedDataForVisualizations().then((hardcodedData) => {
    const data = {
      sheet: hardcodedData.googleSheet,
      settings: hardcodedData.evidenceGapMapSettings,
      chartSettings: hardcodedData.evidenceGapMapChartSettings,
    };
...
async function getHardcodedDataForVisualizations() {
  try {
    const evidenceGapMapChartSettingsResponse = await fetch(
      `https://agricultureinthedigitalage.org/wp-content/uploads/2021/09/evidenceGapMapChartSettings.json`
    );
    const evidenceGapMapChartSettings =
      await evidenceGapMapChartSettingsResponse.json();

    const evidenceGapMapSettingsResponse = await fetch(
      `https://agricultureinthedigitalage.org/wp-content/uploads/2021/09/evidenceGapMapSettings.json`
    );
    const evidenceGapMapSettings = await evidenceGapMapSettingsResponse.json();

    const worldMapSettingsResponse = await fetch(
      `https://agricultureinthedigitalage.org/wp-content/uploads/2021/09/worldMapSettings.json`
    );
    const worldMapSettings = await worldMapSettingsResponse.json();

    const googleSheetResponse = await fetch(
      `https://agricultureinthedigitalage.org/wp-content/uploads/2021/09/googleSheet.json`
    );
    const googleSheet = await googleSheetResponse.json();

    return {
      evidenceGapMapChartSettings,
      evidenceGapMapSettings,
      worldMapSettings,
      googleSheet,
    };
  } catch (error) {
...
function getChartSettingsObjectForEvidenceMap(chartSettings) {
  ...
  resultObj.outcomesBroadCategoriesTitles =
    tempOutcomeBroadCategoriesTitlesArray;
  resultObj.interventionsBroadCategoriesTitles =
    tempInterventionBroadCategoriesTitlesArray;
  resultObj.outcomesNarrowCategoriesTitles =
    tempOutcomeNarrowCategoriesTitlesArray;
  resultObj.interventionsNarrowCategoriesTitles =
    tempInterventionNarrowCategoriesTitlesArray;
