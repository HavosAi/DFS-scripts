...
(function ($) {
  jQuery(document).ready(function () {
    ...
    async function getGoogleSheet(
      ...
      try {
        ...
        const sheet = await fetch(
          responseJSON.feed.entry[numberOfDataTableInGoogleSheet - 1].link[1]
            .href + "?alt=json"
        );
        const sheetJSON = await sheet.json();

        const settings = await fetch(
          responseJSON.feed.entry[numberOfSettingsTableInGoogleSheet - 1]
    ...
    function getStudiesArray(
      ...
      let tempFiltersBlockArray = initializeStudiesValuesArray(
        filtersBlockArray
      );
        ...
        if (index >= numberOfColumns) {
          ...
          if (
            ...
            if (
              ...
              tempStudyCountriesArray.length > 0
            ) {
              let tempObj = {
            ...
            if (
              ...
              filtersValuesFromSheetArrayItem.values.forEach((filterValue) => {
                ...
                if (!isValueAlreadyAdded && filterValue) {
                  let filterSelectOptionElement = document.createElement(
                    "option"
                  );
